...
function assertImagesExist(opts, commitDetails, cb) {
    var calls = _.map(opts.services, function (s) {
...
function buildUrls(opts) {
    ...
    return _.transform(opts.services, function (acc, s) {
        if (s === 'api') {
            if (opts.env == 'live') {
                acc[s] = util.format('https://api%s.socialsuperstore.com', darkStr);
            } else {
                acc[s] = util.format(
                    'https://%s-api%s.socialsuperstore.com', opts.env, darkStr);
            }
        }
        ...
        if (s === 'app') {
            if (opts.env == 'live') {
                ...
                acc[s] = util.format(
                    'https://%s%s.socialsuperstore.com', opts.env, darkStr);
            }
        }
    }, {});
}
...
function kubectl(env, args, cb) {
    async.waterfall([
        git.ensureUpToDate
    ], function (err, res) {
        if (err) { return cb(err); }

        var running = exec('./deploy/kube-wrapper ' + args, {
